var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var service_1 = require("../services/service");
var core_1 = require("angular2/core");
var TodoItem = (function () {
    function TodoItem(todoService) {
        this.todoService = todoService;
        this.removing = false;
        this.toStatus = false;
        this.actions = [];
    }
    TodoItem.prototype.updateStatus = function (item, status) {
        this.todoService.updateStatus(item, status);
    };
    TodoItem.prototype.delete = function (item) {
        this.todoService.removeTask(item);
    };
    __decorate([
        core_1.Input(), 
        __metadata('design:type', Object)
    ], TodoItem.prototype, "item", void 0);
    __decorate([
        core_1.Input(), 
        __metadata('design:type', Object)
    ], TodoItem.prototype, "tasks", void 0);
    __decorate([
        core_1.Input(), 
        __metadata('design:type', Object)
    ], TodoItem.prototype, "typeList", void 0);
    TodoItem = __decorate([
        core_1.Component({
            selector: 'todo-item',
            templateUrl: './directives/item.tpl.html'
        }), 
        __metadata('design:paramtypes', [service_1.TodoService])
    ], TodoItem);
    return TodoItem;
})();
exports.TodoItem = TodoItem;

//# sourceMappingURL=item.js.map
